...
import { CheckBadgeIcon, ChevronDownIcon, ShoppingCartIcon, TruckIcon } from '@heroicons/vue/20/solid'
...
<template>
    <div class="relative h-full overflow-x-auto shadow-md sm:rounded-lg mt-5">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    <tr v-for="order in orderQuery.data.value" :key="order.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="p-4">
                            <img :src="order.temp_url" class="w-24 max-w-full max-h-full rounded" :alt="order.image_path">
                        </td>
                        ...
                        <td v-if="isAdmin" class="relative overflow-visible px-3 py-4 font-semibold text-gray-900 dark:text-white">
                            ...
                            >
                                <Popover v-slot="{ open, close }" class="relative">
                                    ...
                                    <PopoverPanel
                                        ...
                                        >
                                        <div class="overflow-hidden z-[9999]  rounded-lg shadow-lg ring-1 ring-black/5">
                                            <div class="flex flex-col gap-8 bg-white p-7">
                                            <h1
                                                v-for="item in enrichedStatuses"
                                                :key="item.name"
                                                @click="handleStatusChange(order.id, item.id, close)"
                    ...
                    <!-- Empty state message -->
                    <tr v-if="orderQuery.data.value && orderQuery.data.value.length === 0 && !orderQuery.isLoading.value">
