<script lang="ts" setup>
    ...
    // REUPLOAD PAYMENT MUTATION
    const paymentMutation = useMutation({
        mutationFn: async (data: FormData) => {
            const respData = await apiService.post(`/api/reupload/payment/${props.paymentId}`, data)
            return respData
        },
        onSuccess: () => {
            toast.add({
                severity: 'success',
                summary: 'Payment Re-uploaded Successfully',
                life: 1500,
            })

            setTimeout(() => {
                queryClient.invalidateQueries({ queryKey: ['orders'] })
                queryClient.invalidateQueries({ queryKey: ['payments_by_order'] })
                handleCloseModal()
            }, 1000)
        },
...
<template>
    <TransitionRoot appear :show="true" as="template">
        <Dialog as="div" @close="handleCloseModal" class="relative z-[9999]">
                ...
                <div class="flex min-h-full items-center justify-center p-4 text-center">
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                Re-upload Payment Confirmation
                            </DialogTitle>

                            <div class="mt-2">
                                <p class="text-sm text-gray-500">
                                    Product Name: {{ paymentData?.product_name }}
                                </p>
                                <p class="text-sm text-gray-500">
                                    Quantity: {{ paymentData?.total_quantity }}
                                </p>
                                <p class="text-sm text-gray-500">
                            ...
                            <div class="mt-4 flex justify-end gap-3">
                                ...
                                <button
                                    type="button"
                                    :disabled="!file || paymentMutation.isPending.value"
                                    @click="handleReuploadPayment"
                                    :class="[
            ...
            <Loader v-if="paymentMutation.isPending.value" msg="Re-uploading Payment..." />
        </Dialog>
    </TransitionRoot>
</template>
