<script lang="ts" setup>
    ...
    const preferences = ref([{ name: 'realistic' }, { name: 'cartoon' }, { name: 'anime' }, { name: 'painting' }, { name: 'sketch' }])
        ...
        <Dialog as="div" class="relative z-10" @close="handleCloseModal">
            ...
            <div class="fixed inset-0 z-10 overflow-y-auto">
                <div class="flex min-h-full items-center justify-center p-4 text-center">
                    ...
                    >
                        <DialogPanel class="w-full h-[500px] max-w-4xl transform overflow-y-auto bg-white p-8 text-left align-middle shadow-xl transition-all">
                            <button type="button" class="absolute top-4 right-4 text-gray-400 hover:text-gray-500" @click="handleCloseModal">
                            ...
                            <form v-if="imageUrls.length == 0" @submit.prevent="onImageGenerate" class="flex flex-col gap-7 w-full mb-8">
                                ...
                                <div class="flex flex-col gap-2">
                                    ...
                                    <ListSelectBox v-model="style_preference" :options="preferences" displayKey="name" />
                                </div>

                                <button type="submit" class="px-4 py-2 rounded-md bg-black text-white hover:cursor-pointer hover:opacity-75">Generate</button>
                            </form>

                            <div v-if="imageUrls && imageUrls.length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                <div v-for="(imageUrl, index) in imageUrls" :key="'generated-' + index" class="group relative overflow-hidden rounded-md">
                                    <!-- Download Button -->
                                    <a
                                    ...
                                    <!-- Image -->
                                    <img :src="`${aiAPIURL}/generated/image/${imageUrl}`" class="aspect-square w-full rounded-md bg-gray-200 object-cover group-hover:opacity-75 transition" />

                                    <!-- Caption -->
                                    <h3 class="mt-2 text-sm text-center text-gray-700 font-medium">Generated Design {{ index + 1 }}</h3>
                                </div>
                            </div>
                        </DialogPanel>
                    </TransitionChild>
